...
Identical resource to the one in ../../mfast-seqs/resources/ - duplicated
here so the two pipelines (mfast-seqs/ and waldo/) are fully independent
and can be copied/run separately. See that file for the same caveat:
coordinates are hardcoded, approximate centromere/length values and
should be cross-checked against UCSC before diagnostic use.

Output columns (BED, 0-based half-open): chrom start end arm excluded
"""
import argparse
